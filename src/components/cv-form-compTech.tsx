...
const CVFormCompTech: FunctionComponent<Props> = ({ cv }) => {
  ...
  const handleFormChange = (event: React.ChangeEvent<HTMLInputElement>, index: number) => {
    ...
    cv.competenceTechniques[index][event.target.name] = event.target.value;
    setFormFields(data);
  }
  ...
  return (
    <div className="mt-5">
      <h2 className="d-inline me-3">Compétences Techniques</h2>  <button type="button" onClick={addFields} className="btn btn-primary btn-sm">Ajouter une compétence technique</button>
